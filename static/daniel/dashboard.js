...
// * Render trips
// ***************************************************
var upcomingTrips = [], pastTrips = [];
var renderTrips = function () {
  API.getTrips().done(function (userTrips) {
...
var createCard = function (tripObj) {
  ...
  var $cardImg = $('<img>').attr({ src: 'unsplash/api/call?', alt: tripObj.title });
  var $cardTitle = $('<span>').addClass('card-title').text(tripObj.title);
  var $cardContent = $('<div>').addClass('card-content').text(tripObj.location);
